...
INVESTIGATION_ID_PATTERN = re.compile(
    r"^inv_[0-9a-f]{32}$"
)
...
def test_generate_investigation_id_uses_expected_format() -> None:
    ...
    assert INVESTIGATION_ID_PATTERN.fullmatch(
        investigation_id
    )
...
def test_generate_thread_id_uses_expected_format() -> None:
    ...
    assert THREAD_ID_PATTERN.fullmatch(
        thread_id
    )
...
def test_extract_investigation_input_returns_required_fields(
    ...
    result = extract_investigation_input(
...
def set_nested_value(
    ...
    if len(path_parts) != 2:
        raise ValueError(
            f"Unsupported test field path: {field_path}"
        )
