...
The walking skeleton above is complete. The exclusions listed for the Model Service below ("model training", "model inference", "MLflow integration beyond basic service wiring") described that first phase only. The next phase — now also complete — implemented the real training vertical: preprocessing, a stratified train/validation/test split, a controlled candidate comparison (`class_weight=None` vs `balanced`), threshold selection under the `recall >= 0.75` business constraint, MLflow model registration with full provenance, a sealed one-time final test evaluation, Model Service loading the registered artifact from MLflow at startup, and a real `POST /predict` inference endpoint consumed by both the Dashboard's prediction form and the integration smoke test (see `ARCH.md`'s Architecture Status and status tags for exactly what's implemented vs. still target).

The multi-agent supervisor, the promotion workflow, and real PSI/χ² drift computation — all explicitly deferred below — remain deferred; nothing in the training vertical or the `/predict` endpoint changed that.
